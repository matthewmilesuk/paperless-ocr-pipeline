...
- Rebuild the venv against 3.12 if you're not sure what it's running:
  ...
  ```
- **The `admin` / `paperless-dev-2026` account in this dev instance's
  `db.sqlite3` is a throwaway credential from manual testing, not
  anything to replicate.** It was created with `createsuperuser
  --noinput` for one-off convenience during the first real end-to-end
  pipeline run. `db.sqlite3` is gitignored, so it never left this
  machine, but don't copy this username/password into `setup.sh`, docs,
  or a fresh instance elsewhere — every install should go through the
  interactive `createsuperuser` prompt `setup.sh` now runs automatically
  (see "Getting Started" in `README.md`) and get its own, real
  credentials chosen on the spot. If this `db.sqlite3` ever needs
  resetting, deleting it and re-running `setup.sh` is enough; nothing
  depends on this specific account existing.
...
**`run_pipeline()` has now been run once, successfully, end to end
...
`output.deliver_failed()` — see the `output.py` bullet below). Every
stage is also tested in isolation (real tools where it matters —
tesseract/ghostscript/pngquant/verapdf are never mocked; only Google
Document AI is, since that costs real money per call). Don't conflate
"all stages implemented," "one successful real run," and "production
ready" — they're three different claims and only the first two are true
so far.
